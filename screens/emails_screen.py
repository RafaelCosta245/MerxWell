...
class EmailsScreen(BaseScreen):
    ...
    def _create_header_icon(self) -> ft.Control:
        return ft.Icon(name=ft.Icons.EMAIL, color=ft.Colors.RED, size=28)

    def _create_header_title(self) -> ft.Control:
        return ft.Text("E-mails", size=22, weight=ft.FontWeight.BOLD)
    ...
    def _create_content_title(self) -> ft.Control:
        return ft.Text("Área de E-mails", size=18, weight=ft.FontWeight.W_600)

    def _create_content_description(self) -> ft.Control:
        return ft.Text(
            "Tela básica para gerenciamento e disparo de e-mails.",
            size=14,
        )
