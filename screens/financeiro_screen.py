...
class FinanceiroScreen(BaseScreen):
    route: str = "/financeiro"
    ...
    def _create_header_icon(self) -> ft.Control:
        return ft.Icon(name=ft.Icons.ACCOUNT_BALANCE_WALLET, color=ft.Colors.ORANGE, size=28)

    def _create_header_title(self) -> ft.Control:
        return ft.Text("Financeiro", size=22, weight=ft.FontWeight.BOLD)
    ...
    def _create_content_title(self) -> ft.Control:
        return ft.Text("Área Financeira", size=18, weight=ft.FontWeight.W_600)

    def _create_content_description(self) -> ft.Control:
        return ft.Text(
            "Tela básica do Financeiro. Centralize aqui cobranças, recebíveis e indicadores.",
            size=14,
        )
